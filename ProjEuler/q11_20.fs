...
let q13 x = 
  ...
  let getAllProds =
    ...
    let allDownProds = [0..19] |> Seq.collect(fun x -> [0..16] |> Seq.map (fun y -> getProdOf4ConseqAux x y (0, 1) 1 0))
    let allRtDnProds = [0..16] |> Seq.collect(fun x -> [0..16] |> Seq.map (fun y -> getProdOf4ConseqAux x y (1, 1) 1 0))
    let allRtUpProds = [0..16] |> Seq.collect(fun x -> [19..-1..3] |> Seq.map (fun y -> getProdOf4ConseqAux x y (1, -1) 1 0))
  
    allRightProds |> Seq.append (allDownProds) |> Seq.append (allRtDnProds) |> Seq.append(allRtUpProds)

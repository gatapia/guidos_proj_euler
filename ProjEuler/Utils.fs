...
let PRIME_CACHE = File.ReadAllText("primes.txt").Split(',') |> Array.map(fun t -> Int64.Parse(t))
let MAX_PRIME = 3571L;
// HOW CAN THIS BE SLOWER!!!!
let isPrime2 n = 
  if n = 2L || n = 3L then true
  elif n <= 1L || n % 2L = 0L then false
  elif n <= MAX_PRIME then PRIME_CACHE |> Array.exists(fun p -> n = p)
  else
    let max = int64(float n |> sqrt) + 1L
    if max > MAX_PRIME then failwith "Number being tested is too large for cached prime test"
    PRIME_CACHE |> Array.forall (fun p -> p > max || n % p <> 0L)
